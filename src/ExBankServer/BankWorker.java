    ...
    public String checkMessage(String msg) {
        ...
        if (parsedmsg[0].toLowerCase().equals("login") && parsedmsg.length == 2){
            try {
                ...
                id = -1;
                response = invalidAccount.getMessage();
            }
        }

        if (parsedmsg[0].toLowerCase().equals("check") && parsedmsg.length == 2){
            try {
        ...
        if (parsedmsg[0].toLowerCase().equals("deposit") && parsedmsg.length == 3){
        ...
        if (parsedmsg[0].toLowerCase().equals("credit") && parsedmsg.length == 3){
            float amount = Float.parseFloat(parsedmsg[1]);
            try {
        ...
        if (parsedmsg[0].toLowerCase().equals("transfer") && parsedmsg.length == 4){
            ...
            } catch (InvalidAccount | NotEnoughFunds error) {
